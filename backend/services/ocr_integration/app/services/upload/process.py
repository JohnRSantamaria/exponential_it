...
async def save_file(
    ...
    file: UploadFile,
    file_content: bytes,
    uploader_name: UploadersEnum,
    max_retries: int = 3,
):
    file_ext = os.path.splitext(file.filename or "")[-1]
    file_hash = calculate_file_hash(file_content)
    remote_filename = f"{file_hash}{file_ext}"  # hash como nombre
    remote_path = PathBuilder().build(date=invoice.date_invoice)
    full_remote_path = f"{remote_path}/{remote_filename}"

    uploader = get_uploader(name=uploader_name)
